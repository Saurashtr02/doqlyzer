...
class FlowSession(DefaultSession):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Scapy 2.6.1 DefaultSession.__init__ only takes supersession
    ...
    def process(self, packet):
        ...
        if self.packets_count % 100 == 0:
            print(f"Processed {self.packets_count} packets")
        ...
        if self.packets_count % 10000 == 0 or (flow.duration > self.duration_limit and self.output_mode == 'flow') or (self.max_packets > 0 and len(flow.packets) >= self.max_packets):
            print('Packet count: {}'.format(self.packets_count))
            self.garbage_collect(float(packet.time))
    ...
    def garbage_collect(self, latest_time) -> None:
        ...
        for k in keys:
            ...
            if self.output_mode == 'flow':
                if latest_time is None or latest_time - flow.latest_timestamp > EXPIRED_UPDATE or flow.duration > self.duration_limit or (self.max_packets > 0 and len(flow.packets) >= self.max_packets):
                    data = flow.get_data()
                    if self.csv_line == 0:
                ...
                if latest_time is None or latest_time - flow.latest_timestamp > EXPIRED_UPDATE:
                    output_dir = os.path.join(self.output_file, 'doq' if flow.is_doq() else 'ndoq')
...
def generate_session_class(output_mode, output_file, duration_limit, max_packets):
